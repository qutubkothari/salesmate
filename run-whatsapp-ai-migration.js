/**
 * Run WhatsApp AI Migration
 * Creates AI conversation, smart reply, and broadcast tables
 */
...
console.log('🚀 Running WhatsApp AI Migration...\n');

try {
  // Read migration SQL
  const migrationPath = path.join(__dirname, 'migrations', 'create_whatsapp_ai.sql');
  ...
  const tables = db.prepare(`
    ...
    WHERE type='table' AND (
      ...
      name LIKE '%broadcast%' OR 
      name LIKE '%smart_reply%' OR
      name = 'message_templates' OR
      name = 'customer_ai_preferences'
    )
    ORDER BY name
  `).all();
  ...
  console.log('\n✅ WhatsApp AI migration completed successfully!');
