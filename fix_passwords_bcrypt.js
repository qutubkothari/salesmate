...
console.log(`\nRe-importing ${allFsmUsers.length} users with BCRYPT password hashing...\n`);
...
async function reimportUsers() {
  let updated = 0;
  for (const user of allFsmUsers) {
    try {
      const hashedPassword = await hashPassword(user.password);
      updateUser.run(hashedPassword, user.id);
      updated++;
      ...
      console.log(`  ✓ [${tenantName.padEnd(12)}] ${user.role.padEnd(12)} - ${user.name} (${user.phone}) → password: ${user.password}`);
    } catch (err) {
...
reimportUsers().then((result) => {
  console.log(`\n✅ Updated ${result} user passwords with bcrypt hashing`);
  
  // Test one password
  const testUser = db.prepare('SELECT password_hash FROM users WHERE phone = ?').get('8600259300');
  bcrypt.compare('8600259300', testUser.password_hash).then((isMatch) => {
    console.log('\n=== PASSWORD VERIFICATION ===');
    console.log('Test user 8600259300 password matches: ' + (isMatch ? 'YES ✓' : 'NO ✗'));
    db.close();
  });
});
